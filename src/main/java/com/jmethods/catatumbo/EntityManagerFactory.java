...
public class EntityManagerFactory {
     ...
     * @throws IOException if any error occurs such as not able to read the credentials
     *                     file.
     */
     ...
     * @param jsonCredentialsFile the JSON formatted credentials file for the target Cloud
     *                            project.
     ...
     * @param jsonCredentialsFile the JSON formatted credentials file for the target Cloud
     *                            project.
     * @param namespace           the namespace (for multi-tenant datastore) to use. If
     ...
     *
     * @param serviceURL Service URL for the Datastore Emulator. (e.g.
     *                   http://localhost:9999)
     ...
     * @param serviceURL Service URL for the Datastore Emulator. (e.g.
     *                   http://localhost:9999)
     * @param projectId  the project ID. The specified project need not exist in Google
     *                   Cloud.
     * @return an {@link EntityManager} that allows working with the local
     * Datastore (a.k.a Datastore Emulator).
     */
    ...
    public EntityManager createEntityManager(ConnectionParameters parameters) {
        try {
            ...
            httpTransportBuilder.setConnectTimeout(parameters.getConnectionTimeout());
            httpTransportBuilder.setReadTimeout(parameters.getReadTimeout());

            HttpTransportFactory httpTransportFactory = parameters.getHttpTransportFactory();
            if (httpTransportFactory != null) {
                httpTransportBuilder.setHttpTransportFactory(httpTransportFactory);
            }
            datastoreOptionsBuilder.setTransportOptions(httpTransportBuilder.build());
